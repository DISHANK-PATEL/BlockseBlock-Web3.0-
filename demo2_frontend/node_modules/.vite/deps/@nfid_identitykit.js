...
// ../../node_modules/@nfid/identitykit/dist/index.esm.js
init_esm();
var Agent = class _Agent {
  ...
  async call(...params) {
    var _a;
    const delegationTargets = (_a = this.delegation) == null ? void 0 : _a.targets;
    const strategy = this.delegation && (!(delegationTargets == null ? void 0 : delegationTargets.length) || (delegationTargets == null ? void 0 : delegationTargets.find((t) => t.compareTo(Principal.from(params[0])) === "eq"))) ? this.agentStrategy : this.signerAgentStrategy;
  ...
  async createReadStateRequest(...params) {
    var _a, _b;
    return (_b = (_a = this.agentStrategy).createReadStateRequest) == null ? void 0 : _b.call(_a, ...params);
  }
};
...
var IdleManager = class extends TimeoutManager {
  constructor(options = {}) {
...
var SignerClient = class {
  constructor(options) {
    ...
    if (!((_a = options == null ? void 0 : options.idleOptions) == null ? void 0 : _a.disableIdle)) {
  ...
  registerDefaultIdleCallback() {
    var _a, _b, _c;
    if (!((_b = (_a = this.options) == null ? void 0 : _a.idleOptions) == null ? void 0 : _b.disableDefaultIdleCallback)) {
  ...
  async logout(options) {
    ...
    (_a = this.idleManager) == null ? void 0 : _a.exit();
...
var DelegationSignerClient = class _DelegationSignerClient extends SignerClient {
  ...
  static async create(options) {
    ...
    if (!baseIdentity) {
      const createdBaseIdentity = await (!(options == null ? void 0 : options.keyType) || (options == null ? void 0 : options.keyType) === ED25519_KEY_LABEL ? Ed25519KeyIdentity.generate(crypto.getRandomValues(new Uint8Array(32))) : ECDSAKeyIdentity.generate());
    ...
    if (this.shouldCheckIsUserConnected()) {
      ...
      identity = delegationValid ? _DelegationSignerClient.createIdentity(baseIdentity, delegationChain) : new AnonymousIdentity();
      const signerClient2 = new _DelegationSignerClient(options, identity, baseIdentity, options.targets, options.maxTimeToLive);
      if (delegationValid) {
    ...
    const signerClient = new _DelegationSignerClient(options, identity, baseIdentity, options.targets, options.maxTimeToLive);
    return signerClient;
  }
  ...
  async login() {
    ...
    const delegationChain = DelegationChain.fromDelegations(delegationChainResponse.result.signerDelegation.map((delegation) => {
      var _a2;
      return {
        delegation: new Delegation(fromBase64(delegation.delegation.pubkey), BigInt(delegation.delegation.expiration), (_a2 = delegation.delegation.targets) == null ? void 0 : _a2.map((principal) => Principal.fromText(principal))),
    ...
    if (!((_b = (_a = this.options) == null ? void 0 : _a.idleOptions) == null ? void 0 : _b.disableIdle) && !this.idleManager) {
  ...
  initExpirationManager(delegationChain) {
    ...
    if (!this.expirationManager) {
      ...
      (_a = this.expirationManager) == null ? void 0 : _a.registerCallback(async () => {
        await this.logout();
      });
    }
  }
  ...
  async getDelegationType() {
    ...
    if (!delegationChain)
      throw new Error("Not authorized");
    return ((_a = delegationChain.delegations[0].delegation.targets) == null ? void 0 : _a.length) ? DelegationType.ACCOUNT : DelegationType.RELYING_PARTY;
  }
  async getDelegation() {
...
//# sourceMappingURL=@nfid_identitykit.js.map
